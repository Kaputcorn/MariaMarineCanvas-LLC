...
document.addEventListener("DOMContentLoaded", () => {
    const projects = Array.isArray(window.MarysProjects)
    ...
    const prefersReducedMotion =
        window.MarysSite &&
        typeof window.MarysSite.prefersReducedMotion === "boolean"
    ...
    function createImageElement(imageData, options = {}) {
        const {
            lazy = true,
            decorative = false
        } = options;
        ...
        if (imageData && imageData.src) {
            ...
            image.alt = decorative
                ? ""
                : imageData.alt || "Marine project photo";
        ...
        placeholder.textContent = imageData?.placeholder || "Project Photo";
    ...
    function renderImageInsideContainer(container, imageData, options = {}) {
        if (!container) {
            return;
        }

        container.replaceChildren();
        container.classList.remove("placeholder-image");
    ...
    function createProjectCard(project) {
        ...
        card.type = "button";
        card.classList.add("portfolio-card", "reveal");
        ...
        if (project.layout === "wide") {
            card.classList.add("portfolio-card-wide");
        }

        if (project.layout === "tall") {
            card.classList.add("portfolio-card-tall");
        }
        ...
        imageContainer.appendChild(
            createImageElement(firstImage)
        );

        const viewProject = document.createElement("span");
        viewProject.classList.add("portfolio-view-project");
        viewProject.textContent = "View Project";
        imageContainer.appendChild(viewProject);

        if (projectImages.length > 1) {
            const photoCount = document.createElement("span");
            photoCount.classList.add("portfolio-photo-count");
            photoCount.textContent = `${projectImages.length} photos`;
        ...
        if (
            ...
            badge.classList.add("portfolio-before-after-badge");
            badge.textContent = "Before / After";
            imageContainer.appendChild(badge);
        }

        const caption = document.createElement("div");
        caption.classList.add("portfolio-caption");
        ...
        captionText.appendChild(category);
        captionText.appendChild(title);

        const arrow = document.createElement("span");
        arrow.classList.add("portfolio-arrow");
        arrow.textContent = "↗";
    ...
    function updateProjectCount(count) {
        ...
        projectCount.textContent = `Showing ${count} ${label}`;
    }
    ...
    function renderProjects() {
        ...
        const revealElements =
            portfolioGrid.querySelectorAll(".reveal");

        if (
            window.MarysSite &&
            typeof window.MarysSite.observeRevealElements === "function"
        ) {
            window.MarysSite.observeRevealElements(revealElements);
        } else {
    ...
    function getProjectCards() {
        return portfolioGrid.querySelectorAll("[data-project-id]");
    }
    ...
    const projectModalDialog =
        document.querySelector(".project-modal-dialog");
    ...
    const projectModalImageContainer =
        document.getElementById("projectModalImageContainer");
    ...
    const beforeImageContainer =
        document.getElementById("beforeImageContainer");
    const afterImageContainer =
        document.getElementById("afterImageContainer");
    const modalCloseElements =
        document.querySelectorAll("[data-modal-close]");

    /*
        Phase 8.5:
        Give the dynamically populated thumbnail strip an explicit
        semantic grouping so its accessible label is announced.
    */

    if (galleryThumbnails) {
        galleryThumbnails.setAttribute("role", "group");
        galleryThumbnails.setAttribute(
            "aria-label",
    ...
    function renderGalleryImage() {
        ...
        const imageData =
            activeProjectImages[activeImageIndex];
        ...
        if (galleryCounter) {
            galleryCounter.hidden = !hasMultipleImages;
        }
    ...
    function renderGalleryThumbnails() {
        ...
        activeProjectImages.forEach((imageData, index) => {
            ...
            /*
                Phase 8.5:
                The button itself carries the useful image
                description. The image inside the button is
                decorative so screen readers do not announce
                the same information twice.
            */

            thumbnail.setAttribute(
                "aria-label",
                `View photo ${index + 1} of ${activeProjectImages.length}: ${imageDescription}`
            );
    ...
    if (projectModalImageContainer) {
        projectModalImageContainer.addEventListener(
            "touchstart",
            (event) => {
    ...
    function getModalFocusableElements() {
        ...
        return Array.from(
            projectModalDialog.querySelectorAll(
                focusableSelector
            )
        ).filter((element) => {
            return (
